	...
	public boolean response() throws FilterErrorException{
		try {
			if(super.getValue().size() != 2)
				throw new FilterErrorException("The filter 'notIncluded' accepts only 2 numerical values");
			
			if(this.vrfValue > ((Number)super.getValue().get(0)).doubleValue() && this.vrfValue < ((Number)super.getValue().get(1)).doubleValue())
				return false;
			return true;
		} catch (ClassCastException e) {
			throw new FilterErrorException("The filter 'notIncluded' accepts only numerical values");
		}
	}
}
