	...
	public boolean response() throws FilterErrorException {
		try {
			if(super.getValue().size() != 1)
				throw new FilterErrorException("The filter 'lessEqual' accepts only 1 numerical value");
			
			if(this.vrfValue <= ((Number)super.getValue().firstElement()).doubleValue())
				return true;
			return false;
		} catch (ClassCastException e) {
			throw new FilterErrorException("The filter 'lessEqual' accepts only numerical values");
		}
	}
}
